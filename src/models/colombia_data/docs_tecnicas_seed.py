...
SEED_DOCS = [
    ...
    {'area': 'frontend', 'clave': 'doc-front-previsualizador', 'nivel': 'admin', 'orden': 7,
     'titulo': 'Previsualizador en vivo del Diseñador',
     'resumen': 'La vista previa es la tienda real, no una maqueta; refleja los cambios al instante.',
     'contenido': (
        'Cuando personalizas en el Diseñador, la vista previa ya no es un dibujo aproximado: es tu tienda real '
        'cargada dentro del editor, que se repinta al instante con cada cambio (color, logo, textos, menú…) sin '
        'necesidad de guardar. Como es la plantilla publicada de verdad, lo que ves es exactamente lo que verán tus '
        'clientes, y se adapta sola a celular, tablet y computador. Funciona en las 5 plantillas: tienda de '
        'productos (ecommerce), restaurante, taller, catálogo y verde natural.'),
     'tecnico': "v7.0: el preview es un <iframe> con la plantilla real (?preview=1) que recibe el storeConfig en vivo por postMessage type 'TUKO_PREVIEW_CONFIG' y repinta. designer.js: buildConfigToSave() es la fuente unica (guardar + preview); PREVIEW_IFRAME_VERTICALS=['ecommerce','restaurante','taller','catalogo','verde']; previewRealUrl() enruta cada vertical a su plantilla; ensurePreviewIframe()/postPreviewConfig() con handshake TUKO_PREVIEW_READY. Cada plantilla aplica el config: ecommerce applyStoreConfig({skipSplash,skipCategorias}); restaurante/taller applyPreviewConfig() (mapea color/slogan/logo/franja + carta/servicios + direccion/telefono + especialidad-taller al DOM real); catalogo set catConfig.config_tienda+catConfig.color y applyConfig(); verde set negocioData.config_tienda y applyNegocio(). Seguridad: los listeners validan ev.origin===location.origin; contenido del tenant escapado (escapeHtml endurecida + safeImageUrl bloquea javascript:/data:html) en galeria/stats/categorias/hero-badges/nombre (tienda.js), nombre/tel (restaurante/taller) y nombre/logo (verde). Catalogo ya escapado (productos escAttr/escHtml, hero textContent) + hardening anti-inyeccion CSS: safeColor()/safeFont() validan color/fuente antes de interpolarlos en el textContent de <style> (applyTheme/applyFont). Rendimiento: en re-aplicados NO se repite el fetch de categorias ni se relanza el splash, y se limpia el interval del logo. Cache-busting: tienda.js?v=... + bump de SW_VERSION. Bug arreglado de paso: catalogo.js llamaba escapeAttr() (no existe; es escAttr) -> rompia init() del catalogo en produccion. Hallazgo de auditoria: los supuestos 'controles muertos' (whatsapp.cta, filtros, promoBoxes, productDetail, navbar, sidebar) SI estan implementados en la tienda real (applyCTAAvanzado, initFiltrosBar, renderPromoBoxes, etc.)."},

    {'area': 'frontend', 'clave': 'doc-front-sellos-confianza', 'nivel': 'admin', 'orden': 7,
     'titulo': 'Sellos de confianza e insignias en la tienda',
     'resumen': 'La tienda muestra señales de confianza del negocio: la franja superior y las insignias ganadas.',
     'contenido': (
        'Para dar confianza al comprador, la tienda muestra dos cosas de forma automática: (1) una franja de '
        'confianza con verificación, calificación, pedidos entregados, seguidores y antigüedad; y (2) las insignias '
        'que el negocio ha ganado, como pequeñas medallas junto al nombre de la tienda. El dueño no configura nada: '
        'se arman solas con los datos reales del negocio.'),
     'tecnico': "Franja de confianza: assets/tienda/trust-strip.js (autocontenido, incluido en ~10 plantillas) lee /negocio/<id>/trust y muestra verificado/rating/pedidos/seguidores/miembro-desde + conteo de insignias. Insignias del negocio en el storefront ecommerce: tienda.js::renderTrustBadges() (S24) pinta los 3 primeros (tiendaConfig.badges) como mini-medallas junto al nombre: disco con gradiente metalico del color del tier (1 Bronce..5 Diamante), aro e icono bi-* BLANCO + sombra. (Antes eran circulos a 12% de opacidad con icono del mismo color -> casi invisibles; corregido 2026-06-08.) El componente de medallas SVG premium TKMedal (assets/js/badge-medal.js + badge-medal.css) se usa en otras superficies (perfil del negocio, dashboard de gamificacion, modal de celebracion, widget embebible, feed de videos); el storefront usa la mini-medalla inline autocontenida (no carga badge-medal.js, tamaño 26px apropiado al inline). Pendiente menor: creador.html y modulos_crear_tienda/dashboard.html aun renderizan insignias sin TKMedal (este ultimo con datos de demo hardcodeados)."},
