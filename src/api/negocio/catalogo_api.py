...
def guardar_producto():
    ...
    try:
        # LÓGICA DE IDENTIFICACIÓN (Evita el 401 si la cookie falla)
        # Intentamos obtener el usuario de la sesión, si no, del formulario enviado
        user_id = None
        ...
        if not user_id:
            return jsonify({"success": False, "message": "No se detectó usuario activo (401)"}), 401
        ...
        nuevo_producto = ProductoCatalogo(
            ...
            usuario_id=user_id, # Usamos el ID identificado arriba
            activo=True
        )
...
def eliminar_producto(id_producto):
    if request.method == 'OPTIONS':
        return jsonify({"success": True}), 200
    try:
        # Verificamos quién intenta borrar
        user_id = current_user.id_usuario if current_user.is_authenticated else request.form.get('usuario_id')
        ...
        if not prod:
            return jsonify({"success": False, "message": "No encontrado o sin permiso"}), 404
