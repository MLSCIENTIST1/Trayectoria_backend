# ═══════════════════════════════════════════════════════════════════════════════
# TUKOMERCIO - API: Admin Features + Planes v2
# Usa Flask-Login (current_user) + CORS igual que admin_api.py
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
# AUTH (idéntico a admin_api.py - Flask-Login)
# ═══════════════════════════════════════════════════════════════════════════════
...
def admin_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        if not email:
            return build_cors_response({'error': 'No autorizado', 'is_admin': False}, 401)
        is_adm, admin_data = is_admin(email)
        if not is_adm:
            return build_cors_response({'error': 'No eres administrador', 'is_admin': False}, 403)
        g.user_email = email
...
@admin_required
def list_features():
    """Listar features agrupadas por categoría."""
    try:
        ...
        return build_cors_response({'success': True, 'categorias': categorias, 'total': len(features)})
    except Exception as e:
        logger.error(f"Error listando features: {e}")
...
def toggle_feature(feature_id):
    """Toggle activo_global de una feature."""
    try:
        feature = FeatureFlag.query.get(feature_id)
        if not feature:
            return build_cors_response({'error': 'Feature no encontrada'}, 404)
        ...
        return build_cors_response({
            'success': True, 'feature': feature.to_dict(),
            'message': f"Feature '{feature.nombre}' {estado}"
        })
    except Exception as e:
        logger.error(f"Error toggle feature: {e}")
        return build_cors_response({'error': str(e)}, 500)
...
def update_feature(feature_id):
    ...
    try:
        ...
        db.session.commit()
        return build_cors_response({'success': True, 'feature': feature.to_dict()})
    except Exception as e:
        return build_cors_response({'error': str(e)}, 500)
...
def create_feature():
    ...
    try:
        ...
        if not data.get('key') or not data.get('nombre'):
            return build_cors_response({'error': 'key y nombre son obligatorios'}, 400)
        
        if FeatureFlag.query.filter_by(key=data['key']).first():
            return build_cors_response({'error': f"Ya existe feature con key '{data['key']}'"}, 400)
        ...
        return build_cors_response({'success': True, 'feature': feature.to_dict()}, 201)
    except Exception as e:
        return build_cors_response({'error': str(e)}, 500)
...
@admin_required
def list_planes_admin():
    """Listar planes con sus features (admin)."""
    try:
...
def update_plan_features(plan_id):
    """Actualizar features de un plan."""
    try:
        plan = Plan.query.get(plan_id)
        if not plan:
            return build_cors_response({'error': 'Plan no encontrado'}, 404)
        ...
        for fd in features_data:
            ...
            if feature:
                pf = PlanFeature(plan_id=plan.id, feature_id=feature.id,
                                 limite=fd.get('limite'), config_json=fd.get('config', {}))
                db.session.add(pf)
        
        db.session.commit()
        return build_cors_response({
            'success': True, 'plan': plan.to_dict(include_features=True),
...
def assign_plan_to_negocio(negocio_id):
    """Asignar o cambiar plan de un negocio."""
    try:
        data = request.get_json(silent=True) or {}
        plan_key = data.get('plan_key')
        if not plan_key:
            return build_cors_response({'error': 'plan_key es obligatorio'}, 400)
        
        plan = Plan.query.filter_by(key=plan_key, activo=True).first()
        if not plan:
            return build_cors_response({'error': f"Plan '{plan_key}' no encontrado"}, 404)
        ...
        if not negocio:
            return build_cors_response({'error': 'Negocio no encontrado'}, 404)
        ...
        nuevo_plan = NegocioPlan(
            ...
            asignado_por='admin', notas=data.get('notas', f'Asignado por {g.user_email}')
        )
        ...
        logger.info(f"📊 Negocio {negocio_id} ({negocio[1]}) → Plan '{plan.nombre}' por {g.user_email}")
        
        return build_cors_response({
            'success': True, 'negocio_id': negocio_id, 'negocio_nombre': negocio[1],
            'plan': plan.to_dict(), 'message': f"Plan '{plan.nombre}' asignado a '{negocio[1]}'"
        })
    except Exception as e:
        return build_cors_response({'error': str(e)}, 500)
...
def list_negocios_with_plans():
    ...
    try:
        ...
        return build_cors_response({
            'success': True,
            'negocios': [{
                'id': n[0], 'nombre': n[1], 'slug': n[2],
                'plan_key': n[3] or 'basic', 'activo': n[4], 'ciudad': n[5],
                'plan_nombre': n[6] or 'Basic', 'plan_color': n[7] or '#22c55e',
                'plan_icono': n[8] or '🌱'
            } for n in negocios],
...
def get_my_features():
    """Features disponibles para el negocio actual."""
    try:
        ...
        if not negocio_id:
            return build_cors_response({'error': 'negocio_id requerido'}, 400)
        ...
        ).join(PlanFeature, PlanFeature.feature_id == FeatureFlag.id
        ...
        for f in all_features:
            ...
            features_response.append({
                'key': f.key, 'nombre': f.nombre, 'categoria': f.categoria,
                'icono': f.icono, 'visible': f.visible,
        ...
        return build_cors_response({'success': True, 'plan': plan_key, 'features': features_response})
    except Exception as e:
        return build_cors_response({'error': str(e)}, 500)
...
def check_feature(feature_key):
    ...
    try:
        ...
        return build_cors_response(result)
    except Exception as e:
        return build_cors_response({'error': str(e)}, 500)
...
def list_public_planes():
    """Listar planes disponibles (público, sin auth)."""
    try:
