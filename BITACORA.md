...
- **RESUELTO (2026-06-24):** `pedidos.html` ahora trae al iniciar el WhatsApp real (`cargarTelNegocio()` → `GET /negocio/slug/<slug>` → `config_tienda.whatsapp.numero` → fallback `negocio.whatsapp`/`telefono`) y lo cachea en `asesor_tel`; se auto-sana en cada carga. Los 5 mensajes de estado ya no muestran el `3058916907` por defecto sino el del Designer. Verificado: rodar = `+573228188375`.
...
- **Verificado (reposición de stock al cancelar):** el stock se descuenta al **aceptar/confirmar** el pedido online (`notifications_negocio_api.py` → `MovimientoStock` SALIDA). Al **cancelar**, `pedidos_api.py` repone automáticamente (movimiento ENTRADA, suma las unidades) usando **la misma clave de producto** (`id`/`id_producto`) y cantidad → repone exactamente lo descontado. Cubre `confirmado` y (gracias al cambio de hoy) `preparando`. Para enviados/entregados, la **Devolución** repone con la casilla "reingresar al inventario" (misma lógica). No requirió cambios nuevos; quedó confirmado por revisión de código.
...
- **Fix (`designer.js`):** `showImagePreview(id, url, target)` ahora acepta un `target` opcional y pinta un botón **"🗑 Eliminar"**; `removeSplashImage(target)` limpia la config (`splash.backgroundImage` o `splash.images[i] = null`) + el preview y llama `updatePreview()`. Se pasa el target tanto al **subir** como al **cargar** la config (fondo + 3 logos). `designer.js?v=20260619-splashdel`. JS validado.
- **Ampliado (mismo día):** botón Eliminar también en **logo principal** (X sobre el preview), **hero/banner** y **imagen para compartir (OG)**. `removeSplashImage` extendido a `heroBanner`/`logo`/`seoOgImage`; `showImagePreview` limpia el preview si la URL viene vacía. Slider y galería ya tenían su X por ítem (no se tocaron). `designer.js?v=20260619-imgdel`.
...
- **FIX raíz (worker 1.32):** algunos productos salían SIN preview (ni imagen genérica). Causa: `buildOgHtml` (`_worker.js`) interpolaba nombre/descripción **sin escapar**; descripciones de marketing con saltos de línea, `*`, comillas `"` o `<`/`>` rompían el atributo `content="..."` → WhatsApp no parseaba la tarjeta. Fix: `_ogEsc` (`& < > "`) + `_ogClean` (colapsa a 1 línea, recorta ~200) en title/description/canonical/imagen; redirect `<script>` con `JSON.stringify`. Verificado en vivo: producto 1342 (descr. larga) ahora devuelve og:title/description/image correctos. Nota: WhatsApp cachea la preview por URL → para ver el cambio en URLs ya compartidas, usar una URL nueva (`&v=2`).

---

## 2026-06-09 — 🔴 INCIDENTE infra: Neon excedió cuota de transferencia (app caída)

- **Síntoma:** inventario/pedidos/dashboard/contabilidad no cargan (500) en el camino autenticado; `/api/health` OK (estático).
- **Causa real (no código):** `psycopg2.OperationalError ... Neon ... "Your project has exceeded the data transfer quota. Upgrade your plan to increase limits."` — confirmado golpeando `/api/negocio/slug/rodar` (500) vs `/api/health` (200).
- **Acción del dueño:** subir plan de Neon (Launch ~USD $19/mes) o esperar reinicio del ciclo. Mitigación futura: cachear lecturas frecuentes, evitar `SELECT *` en listados.
